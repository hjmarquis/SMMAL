...
grid.file = "simresult/hd_misOR_grid.rda"
...
if(file.exists(grid.file))
{
  ...
  for (i in 1:nseq)
  {
    ...
    dat = sim.gen.hd(N, p, 
                     ...
                     par.lp.Y1 = list(coef1=coef.b1,coef2 = coef2),
                     par.lp.Y0 = list(coef1=coef.b0,coef2 = coef2),
    ...
    sim.np <- foreach(ipar = 1:(Nrep*3),.packages = pak.list ) %dopar%
      {
        ...
        data.file =  paste0("tmpdat/hd_misOR_", setting, "_dat_rep", irep, ".rda")
        res.file = paste0("tmpresult/hd_misOR_", setting, "_res_rep",
                          irep,'L', iL,".rda")
        res.file.rep = paste0("tmpresult/hd_misOR_", setting, "_res_rep",
                          irep,'L', 1:3,".rda")
...
res.file.all = paste0("tmpresult/hd_misOR_", setting, "_res_rep",
...
if(all(file.exists(res.file.all)))
{
  ...
  save(sim.np, file = paste0("simresult/hd_misOR_",
                             setting,"_v3.rda"))
