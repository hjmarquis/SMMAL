...
grid.file = "simresult/hd_correct_grid.rda"
    ...
    sim.np <- foreach(ipar = 1:(Nrep*3),.packages = pak.list ) %dopar%
      {
        ...
        data.file =  paste0("tmpdat/hd_correct_", setting, "_dat_rep", irep, ".rda")
        res.file = paste0("tmpresult/hd_correct_", setting, "_res_rep",
                          irep,'L', iL,".rda")
        res.file.rep = paste0("tmpresult/hd_correct_", setting, "_res_rep",
                          irep,'L', 1:3,".rda")
...
res.file.all = paste0("tmpresult/hd_correct_", setting, "_res_rep",
...
if(all(file.exists(res.file.all)))
{
  ...
  save(sim.np, file = paste0("simresult/hd_correct_",
                             setting,"_v3.rda"))
