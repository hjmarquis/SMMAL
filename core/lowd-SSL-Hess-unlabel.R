SSL.Hess.unlabel = function(Y,trt,X,S, XN,SN)
{
  n = length(Y)
  ...
  eta1 = coef(glm(Y*trt~cbind(X,S,1/ps),family = binomial)) 
  eta0 = coef(glm(Y*(1-trt)~cbind(X,S,1/(1-ps)),family = binomial)) 
  
  ps1 = ps 
  ps0 = 1-ps
  pred1 = expit(drop(beta1[1]+X%*% beta1[-1]))
  pred0 = expit(drop(beta0[1]+X%*% beta0[-1]))
  imp1 = expit(drop(eta1[1]+cbind(X,S,1/ps1)%*% eta1[-1]))
  imp0 = expit(drop(eta0[1]+cbind(X,S,1/ps0)%*% eta0[-1]))
  Ut = (-pred1)/ps1 + (-pred0)/ps0
  ...
  S.eta1 =( trt*Y-imp1)*cbind(IX,S,1/ps1)
  S.eta0 = ((1-trt)*Y-imp0)*cbind(IX,S,1/ps0)
  ...
  Ut = (-pred1)/ps + (-pred0)/(1-ps) 
  impt = expit(drop(gr[1]+cbind(XN,SN,Ut)%*% gr[-1]))
  
  ate = mean(pred1 - pred0 + (imp1-impt*pred1)/ps - (imp0-(1-impt)*pred0)/(1-ps))
...
# gradient of ate
  grad.alpha = apply(((imp1-impt*pred1)*dps1-(imp0-(1-impt)*pred0)*dps0
                      + dimp1/ps1*eta1u*dps1 - dimp0/ps0*eta0u*dps0
                      + Ut*dimpt*grUt*(-pred1*dps1-pred0*dps0)
  )*IX,2,mean)
  ...
  grad.gr = apply(Ut*impt*(1-impt)*cbind(IX,SN,Ut),2,mean)
  grad.eta1 = apply(imp1*(1-imp1)/ps*cbind(IX,SN,1/ps),2,mean)
  grad.eta0 = -apply(imp0*(1-imp0)/(1-ps)*cbind(IX,SN,1/(1-ps)),2,mean)
  
  grad = c(grad.alpha,grad.beta1,grad.beta0,grad.gr,grad.eta1,grad.eta0)
  
# Hessian with unlabelled data
  ...
  H.gr.alpha = rbind(
    t(cbind(IX,S)) %*% (impt*(1-impt)*grUt*
                          (-(-pred1)/expps+(-pred0)*expps)*IX)/n, 
    apply((Ut*impt*(1-impt)*grUt-(trt-impt))*
            (-(-pred1)/expps+(-pred0)*expps)*IX,
          2, mean)
  )
  H.gr.beta1 = rbind(
    t(cbind(IX,S)) %*% (impt*(1-impt)*grUt*
  ...
  H.eta1 = t(cbind(IX,S,1/ps)) %*% (imp1*(1-imp1)*cbind(IX,S,1/ps))/n
  eta1u = rev(eta1)[1]
  H.eta1.alpha = rbind(
    t(cbind(IX,S)) %*% (imp1*(1-imp1)*eta1u/(-expps)*IX)/n,
    apply((imp1*(1-imp1)/ps*eta1u - (trt*Y-imp1))/(-expps)*IX,
          2,mean)
  )
  H.eta0 = t(cbind(IX,S,1/(1-ps))) %*% (imp0*(1-imp0)*cbind(IX,S,1/(1-ps)))/n
  eta0u = rev(eta0)[1]
  H.eta0.alpha = rbind(
    t(cbind(IX,S)) %*% (imp0*(1-imp0)*eta0u*expps*IX)/n,
    apply((imp0*(1-imp0)/(1-ps)*eta0u - ((1-trt)*Y-imp0))*expps*IX,
          2,mean)
  )
