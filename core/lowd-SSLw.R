SSLw.aipw = function(Y,trt,X,S, XN,SN)
{
  n = length(Y)
  ...
  eta1 = coef(glm(trt*Y~cbind(X,S),family = binomial,
                  weights = 1/ps)) 
  eta0 = coef(glm((1-trt)*Y~cbind(X,S),family = binomial,
                  weights = 1/(1-ps))) 
  # 
  # pred1 = expit(drop(beta1[1]+X%*% beta1[-1]))
  # pred0 = expit(drop(beta0[1]+X%*% beta0[-1]))
  # imp1 = expit(drop(eta1[1]+cbind(X,S)%*% eta1[-1]))
  # imp0 = expit(drop(eta0[1]+cbind(X,S)%*% eta0[-1]))
  # wt = (pred1)/ps + (pred0)/(1-ps)
  # gr = coef(glm(trt~cbind(X,S),family = binomial,weights = wt))
  # impt =  expit(drop(gr[1]+cbind(X,S)%*% gr[-1]))
  # 
  # IX = cbind(1,X)
  # S.alpha = (trt-ps)*IX
  # S.beta1 = trt*(Y-pred1)*IX
  # S.beta0 = (1-trt)*(Y-pred0)*IX
  # S.gr = (trt-impt)*wt*cbind(IX,S)
  # S.eta1 = (trt*Y-imp1)/ps*cbind(IX,S)
  # S.eta0 = ((1-trt)*Y-imp0)/(1-ps)*cbind(IX,S)
  # 
  ...
  imp1 = expit(drop(eta1[1]+cbind(XN,SN)%*% eta1[-1]))
  imp0 = expit(drop(eta0[1]+cbind(XN,SN)%*% eta0[-1]))
  wt = (pred1)/ps + (pred0)/(1-ps) 
  impt = expit(drop(gr[1]+cbind(XN,SN)%*% gr[-1]))
  
  ate = mean(pred1 - pred0 + (imp1-impt*pred1)/ps - (imp0-(1-impt)*pred0)/(1-ps))
  
  # IX = cbind(1,XN)
  # expps = exp(drop(alpha[1]+XN %*% alpha[-1]))
  # grad.alpha = - apply((1/expps*impt*(imp1-pred1)+expps*(1-impt)*(imp0-pred0))*IX,2,mean)
  # grad.beta1 = apply((1-impt/ps)*pred1*(1-pred1)*IX,2,mean)
  # grad.beta0 = -apply((1-(1-impt)/(1-ps))*pred0*(1-pred0)*IX,2,mean)
  # grad.gr = apply(Ut*impt*(1-impt)*cbind(IX,SN,Ut),2,mean)
  # grad.eta1 = apply(impt/ps*imp1*(1-imp1)*cbind(IX,SN,1/ps),2,mean)
  # grad.eta0 = -apply((1-impt)/(1-ps)*imp0*(1-imp0)*cbind(IX,SN,1/(1-ps)),2,mean)
  # 
